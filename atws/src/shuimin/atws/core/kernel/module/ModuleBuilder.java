...
{
	public static void build(Resource rootResource, String mid, String mpkgname)
	{
...
// }
/**
 * module builder from classpath (.class as file in filesystem)
 * 
 * @author ed
 * 
 */
class ModuleBuilderCP
{
	...
	// .getProperty(Const.SETTINGS_KEY_RESOURCE_FILE_SUFFIXES);
	...
	ModuleBuilderCP(String mid, String mpkgname)
	{
		...
		this.absModulePath = ctx.ROOT_PATH + "/" + pkgName.replaceAll("\\.", "/");
	...
	private void _build_recur(Resource parent_resource, File f, String classNamePrefix)
	{
		A._assert(f, "null dir");
		logger.debug("building DirResource under pkg: " + classNamePrefix);
		/* reading content */
		for (File clazzF : f.listFiles()) {
			...
			if (clazzF.isDirectory()) {
				...
				A._assert(dirResource, "resource null, build not success");
				dirResource.name(clazzF.getName());
				_build_recur(dirResource, clazzF, classNamePrefix + "." + clazzF.getName());
				continue;
			}
			if (clazzF.isFile() && StrUtils.notBlank(clazzF.getName())) {
				/* build as class */
				if (clazzF.getName().endsWith(".class")) {
					String classFullName = classNamePrefix + "." + clazzF.getName();
					logger.debug("find class :" + classFullName);
					_buildRegularResource(parent_resource, classFullName);
					continue;
				}
				/* build as File */
				// String[] name = F.splitSuffixName(f);
				// if (name[1] != null && -1 !=
				// A.ArrayContains(suffixes, name[1])) {
				// _buildFileResource(parent_resource, f);
				// }
				if (StrUtils.notBlank(f.getName())) {
					_buildFileResource(parent_resource, f);
				}
			}
		}
	...
	private void _buildFileResource(Resource parent, File f)
	{
		...
		try {
			ResourceFactory.build("fileResourceBuilder", new Object[] { parent, f });
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	private DefaultModule _buildModule(Resource root)
	{
		...
		try {
			DefaultModule m = (DefaultModule) ResourceFactory.build("directoryResourceBuilder",
				new Object[] { root, DefaultModule.class });
	...
	{
		try {
			return (DirectoryResource) ResourceFactory.build("directoryResourceBuilder", new Object[] {
				parent,
	...
	private void _buildRegularResource(Resource parent, String className)
	{
		try {
			...
			ResourceFactory.build("regularResourceBuilder", new Object[] { parent, c });
		}
		catch (Exception e) {
			e.printStackTrace();
			logger.err(e);
